...
void rectangle::draw() {
    ...
    glVertex2f((position + anchor * 0.5 + (anchor * 0.5 << ang * (-2))).getX(),
               (position + anchor * 0.5 + (anchor * 0.5 << ang * (-2))).getY());
...
void UFO::draw() {
    ...
    glVertex2f(position.getX() + (anchor * 1.5 << 0.333 * PI).getX(),
               position.getY() + (anchor * 1.5 << 0.333 * PI).getY());
    glVertex2f(position.getX() + (anchor << 0.667 * PI).getX(), position.getY() + (anchor << 0.667 * PI).getY());
    glVertex2f(position.getX() + (anchor * 1.5 << 0.667 * PI).getX(),
               position.getY() + (anchor * 1.5 << 0.667 * PI).getY());
    glEnd();
